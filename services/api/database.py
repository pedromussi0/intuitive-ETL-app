...
if not os.path.exists(DOTENV_PATH):
    ...
    if not dotenv_loaded: logger.error(f"python-dotenv reported FAILURE loading file: {DOTENV_PATH}")
    else: logger.info(f"Successfully loaded variables from {DOTENV_PATH}")
# ---
...
try:
    ...
    logger.info(f"Loaded DB settings: User={settings.db_user}, Host={settings.db_host}, Port={settings.db_port}, DB={settings.db_name}")
except ValidationError as e:
    logger.error(f"CRITICAL: Failed to load/validate database settings AFTER loading .env.")
    logger.error(f"Ensure required variables specified by validation_alias (e.g., POSTGRES_PASSWORD) exist in {DOTENV_PATH}.")
...
async def connect_db():
    global pool
    if pool: return
    logger.info(f"Attempting to connect to database: postgresql://{settings.db_user}:***@{settings.db_host}:{settings.db_port}/{settings.db_name}")
    try:
        ...
        async with pool.acquire() as connection:
             ...
             if val == 1: logger.info("Database connectivity test successful.")
             else: logger.warning("Database connectivity test returned unexpected value.")
...
async def disconnect_db():
    ...
    if pool:
        ...
        except Exception as e: logger.exception(f"Error occurred while closing database pool: {e}")
        finally:
             pool = None
...
async def get_db_pool():
    if pool is None:
        logger.error("get_db_pool called when connection pool is None. Check application startup and connection logic.")
