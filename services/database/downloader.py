...
# --- Configuration ---
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(__file__))) # Navigate up to project root
...
def download_file(url, target_path):
    ...
    try:
        ...
        logging.error(f"HTTP error occurred while downloading {url}: {http_err} - Status Code: {http_err.response.status_code}")
        return False
...
def download_accounting_data(base_url, target_dir, years_to_download):
            ...
            for link in dir_soup.find_all('a'):
                ...
                if href and href.lower().endswith('.zip'):
                    ...
                    if not href.startswith('?') and href != '../':
                         zip_links.append(urljoin(year_url, href)) # Construct absolute URL
            ...
            for zip_url in zip_links:
                ...
                if download_file(zip_url, zip_path):
                    if unzip_file(zip_path, target_dir):
                        year_downloaded_successfully = True # Mark success if at least one file is processed
                    else:
                        logging.warning(f"Could not unzip {zip_filename}. Please check the file manually.")
                else:
                    logging.warning(f"Failed to download {zip_filename}.")

            if year_downloaded_successfully:
                 downloaded_count += 1 # Increment count if any file from the year was processed

        except requests.exceptions.HTTPError as http_err:
            if http_err.response.status_code == 404:
                logging.warning(f"Directory for year {year} not found (404): {year_url}")
            else:
    ...
    logging.info(f"Finished accounting data download attempt. Processed data for {downloaded_count} year(s).")
...
def download_operator_data(base_url, target_dir):
    ...
    try:
        ...
        if not found:
            for link in links:
                ...
                if href and href.lower().endswith('.csv'):
                    csv_link = urljoin(base_url, href)
                    logging.info(f"Specific operator file '{known_filename}' not found, using first CSV found: {os.path.basename(csv_link)}")
                    break
        ...
        logging.error(f"An unexpected error occurred during operator data download: {e}")
