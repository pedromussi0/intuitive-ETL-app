...
def get_db_connection():
    ...
    try:
        ...
        logging.error(f"Check connection parameters: Host={DB_HOST}, Port={DB_PORT}, DB={DB_NAME}, User={DB_USER}")
        raise
        ...
        with open(file_path, mode='r', encoding=FILE_ENCODING) as csvfile:
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                    ...
                    reg_ans_str = row.get('Registro ANS') # Handle potential space in header
                    ...
                    if reg_ans is None:
                        logging.warning(f"Skipping row {row_num} due to missing or invalid Registro_ANS: {row}")
                        skipped_count += 1
                        continue
                    ...
                    data_tuple = (
                        ...
                        row.get('UF'), row.get('CEP'), row.get('DDD'), row.get('Telefone'), row.get('Fax'),
                        row.get('Endereco_eletronico'), row.get('Representante'), row.get('Cargo_Representante'),
                        data_reg_ans
                    )

                    if len(data_tuple) != len(sql_columns):
                         raise ValueError(f"Row {row_num}: Mismatch between data tuple size ({len(data_tuple)}) and expected columns ({len(sql_columns)})")
...
def import_demonstracoes_batch(conn, file_path, valid_ans_set, batch_size=1000): # Added valid_ans_set parameter
    """
    ...
    try:
        ...
        with open(file_path, mode='r', encoding=FILE_ENCODING) as csvfile:
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                try:
                    ...
                    if data_dt is None or conta_contabil is None or descricao is None:
                        logging.warning(f"Skipping row {row_num} in {base_filename} due to missing required fields (DATA, CD_CONTA_CONTABIL, or DESCRICAO): {row}")
                        skipped_other += 1
                        continue

                    # Prepare tuple if all checks pass
                    data_tuple = (data_dt, reg_ans, conta_contabil, descricao, vl_saldo_inicial, vl_saldo_final)
                    batch.append(data_tuple)

                except (ValueError, TypeError, KeyError) as data_error:
                    logging.error(f"Data processing error on row {row_num} in {base_filename} (REG_ANS: {reg_ans}): {row} - Error: {data_error}")
                    skipped_other += 1
                    continue # Skip this row
                except Exception as proc_error:
                     logging.error(f"Unexpected row processing error on row {row_num} in {base_filename} (REG_ANS: {reg_ans}): {row} - Error: {proc_error}")
                     skipped_other += 1
                     continue # Skip this row
                ...
                if len(batch) >= batch_size:
                    try:
                        ...
                        logging.debug(f"Committed batch of {len(batch)} rows for {base_filename}")
                        batch = []
                        ...
                        logging.error(f"Unexpected error during batch execution for {base_filename} (near row {row_num}): {batch_exec_err}")
                        conn.rollback()
                        logging.warning(f"Transaction rolled back for {base_filename} due to unexpected batch error.")
                        file_succeeded = False
                        break
            ...
            if file_succeeded and batch:
                try:
                    ...
                    conn.commit()
                    inserted_count += len(batch)
                    logging.debug(f"Committed final batch of {len(batch)} rows for {base_filename}")
                except (psycopg2.DatabaseError, psycopg2.InterfaceError) as db_err:
                    logging.error(f"Database error during final batch insert for {base_filename}: {db_err}")
                    conn.rollback()
                    logging.warning(f"Transaction rolled back for final batch of {base_filename}.")
                    file_succeeded = False
                except Exception as final_batch_err:
                     logging.error(f"Unexpected error during final batch execution for {base_filename}: {final_batch_err}")
                     conn.rollback()
                     logging.warning(f"Transaction rolled back for final batch of {base_filename} due to unexpected error.")
                     file_succeeded = False
        ...
        logging.log(log_level, f"Finished processing {base_filename}. Success: {file_succeeded}. "
...
if __name__ == "__main__":
    ...
    try:
        ...
        if operator_files:
            ...
            logging.warning(f"No operator file matching '{OPERATOR_FILE_PATTERN}' found in {DATA_DIR}")
        ...
        try:
            ...
            cursor.close()
            logging.info(f"Loaded {len(valid_ans_set)} valid REG_ANS values from operadoras table into memory.")
        ...
        if accounting_files:
            ...
            try:
                ...
                logging.info("Demonstracoes_contabeis table truncated.")
                 ...
                 logging.error(f"Failed to truncate demonstracoes_contabeis: {trunc_error}")
            ...
            successful_files = 0
            failed_files = 0
            for acc_file in sorted(accounting_files): # Sort ensures some order if needed
                if import_demonstracoes_batch(connection, acc_file, valid_ans_set=valid_ans_set): # Use batch import
                    successful_files +=1
                else:
                    failed_files += 1
            logging.info(f"Accounting file import summary: Successful={successful_files}, Failed={failed_files}")

        else:
             logging.warning(f"No accounting files matching '{ACCOUNTING_FILE_PATTERN}' found in {DATA_DIR}")
